...
import { toWorkspaceVisibleUploadPath } from './session-paths';
...
export class ClaudeCodeWorker implements AgentExecutor {
      ...
      const manifest = message.attachments
        .map((attachment) => {
          const location = attachment.localPath
            ? this.toPromptAttachmentLocation(session.workingDirectory, attachment.localPath) ?? attachment.url
            : attachment.url;
  ...
  private toPromptAttachmentLocation(workingDirectory: string, candidatePath: string): string | null {
    if (this.isInsideWorkingDirectory(workingDirectory, candidatePath)) {
      return relative(workingDirectory, candidatePath);
    }

    const workspaceVisibleUploadPath = toWorkspaceVisibleUploadPath(workingDirectory, candidatePath);
    if (workspaceVisibleUploadPath) {
      return workspaceVisibleUploadPath;
    }
