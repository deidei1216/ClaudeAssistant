...
- `core/worker.ts` drives `Codex` CLI execution.
...
- `sessions/<sessionId>/workspace/` is Codex's working area for scratch files, edits, and intermediate artifacts.
...
- `/cd <path>` changes the Codex working directory for the current session.
