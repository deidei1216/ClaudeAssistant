...
import { resolveSessionUploadsDirectory, toWorkspaceVisibleUploadPath } from './session-paths';
...
export function createRecentFileRecord(input: {
  ...
  if (input.source === 'discord_inbound') {
    if (!isInsideWorkingDirectory(workingDirectory, absolutePath) && !isInsideWorkingDirectory(uploadsDirectory, absolutePath)) {
      throw new Error(`Recent inbound file is outside the session boundary: ${absolutePath}`);
    }
  } else if (!isWithinPublishedDeliveriesBoundary(workingDirectory, absolutePath)) {
    throw new Error(`Recent file is outside the deliveries boundary: ${absolutePath}`);
  }

  const relativePath = input.source === 'discord_inbound'
    ? (toWorkspaceVisibleUploadPath(workingDirectory, absolutePath) ?? relative(workingDirectory, absolutePath).replace(/\\/g, '/'))
    : relative(workingDirectory, absolutePath).replace(/\\/g, '/');
